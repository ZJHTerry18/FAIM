...
import data.transforms_with_mask as TwM
from torch.utils.data import DataLoader
from data.dataloader import DataLoaderX
from data.dataset_loader import ImageDataset, ImageDatasetWithMask, VideoDataset
from data.samplers import DistributedRandomIdentitySampler, DistributedInferenceSampler
from data.datasets.ltcc import LTCC, LTCC_SC
...
def build_img_transforms(config):
    if config.MODEL.PATCH_SHUFFLE:
        transform_train = TwM.Compose([
            TwM.Resize((config.DATA.HEIGHT, config.DATA.WIDTH)),
            TwM.RandomCroping(p=config.AUG.RC_PROB),
            TwM.RandomHorizontalFlip(p=config.AUG.RF_PROB),
            TwM.ToTensor(),
            TwM.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            TwM.RandomClothesErasing(p=config.AUG.RCE_PROB),
            TwM.RandomClothesErasingBoth(p=config.AUG.RCEB_PROB),
            TwM.RandomClothesErasingEither(p=config.AUG.RCEE_PROB),
            TwM.RandomErasing(p=config.AUG.RE_PROB)
        ])
    else:
        transform_train = TwM.Compose([
            TwM.Resize((config.DATA.HEIGHT, config.DATA.WIDTH)),
...
def build_dataloader(config):
    ...
    # image dataset
        ...
        train_sampler = DistributedRandomIdentitySampler(dataset.train, 
                                                         num_instances=config.DATA.NUM_INSTANCES, 
                                                         seed=config.SEED)
        trainloader = DataLoaderX(dataset=ImageDatasetWithMask(dataset.train, transform=transform_train),
                                 sampler=train_sampler,
                                 batch_size=config.DATA.TRAIN_BATCH, num_workers=config.DATA.NUM_WORKERS,
                                 pin_memory=True, drop_last=True)
