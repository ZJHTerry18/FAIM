...
class LaST(object):
    ...
    def __init__(self, root='data', train_with_mask=True, test_with_mask=False, **kwargs):
        ...
        self.val_query_mask_dir = osp.join(self.dataset_dir, 'human_parsing', 'val', 'query')
        self.val_gallery_mask_dir = osp.join(self.dataset_dir, 'human_parsing', 'val', 'gallery')
        self.test_query_mask_dir = osp.join(self.dataset_dir, 'human_parsing', 'test', 'query')
        self.test_gallery_mask_dir = osp.join(self.dataset_dir, 'human_parsing', 'test', 'gallery')
        self._check_before_run()

        pid2label, clothes2label, pid2clothes = self.get_pid2label_and_clothes2label(self.train_dir)

        train, num_train_pids = self._process_dir(self.train_dir, 'train', pid2label=pid2label, clothes2label=clothes2label, relabel=True)
        val_query, num_val_query_pids = self._process_dir(self.val_query_dir, 'val_query', relabel=False)
        val_gallery, num_val_gallery_pids = self._process_dir(self.val_gallery_dir, 'val_gallery', relabel=False, recam=len(val_query))
        test_query, num_test_query_pids = self._process_dir(self.test_query_dir, 'test_query', relabel=False)
        test_gallery, num_test_gallery_pids = self._process_dir(self.test_gallery_dir, 'test_gallery', relabel=False, recam=len(test_query))
    ...
    def _check_before_run(self):
        ...
        if not osp.exists(self.val_query_mask_dir):
            raise RuntimeError("'{}' is not available".format(self.val_query_mask_dir))
        if not osp.exists(self.val_gallery_mask_dir):
            raise RuntimeError("'{}' is not available".format(self.val_gallery_mask_dir))
        if not osp.exists(self.test_query_mask_dir):
            raise RuntimeError("'{}' is not available".format(self.test_query_mask_dir))
        if not osp.exists(self.test_gallery_mask_dir):
            raise RuntimeError("'{}' is not available".format(self.test_gallery_mask_dir))

    def _process_dir(self, dir_path, dataset_name, pid2label=None, clothes2label=None, relabel=False, recam=0):
        ...
        for ii, img_path in enumerate(img_paths):
            ...
            if relabel and clothes2label is not None:
                ...
                clothes_id = pid
            if dataset_name == 'train' and self.train_with_mask:
                mask = os.path.join(self.train_mask_dir, img_path.split('/')[-2], img_path.split('/')[-1][:-4] + '.npy')
                dataset.append((img_path, pid, camid, clothes_id, mask))
            elif dataset_name == 'val_query' and self.test_with_mask:
                mask = os.path.join(self.val_query_mask_dir, img_path.split('/')[-2], img_path.split('/')[-1][:-4] + '.npy')
                dataset.append((img_path, pid, camid, clothes_id, mask))
            elif dataset_name == 'val_gallery' and self.test_with_mask:
                mask = os.path.join(self.val_gallery_mask_dir, img_path.split('/')[-2], img_path.split('/')[-1][:-4] + '.npy')
                dataset.append((img_path, pid, camid, clothes_id, mask))
            elif dataset_name == 'test_query' and self.test_with_mask:
                mask = os.path.join(self.test_query_mask_dir, img_path.split('/')[-2], img_path.split('/')[-1][:-4] + '.npy')
                dataset.append((img_path, pid, camid, clothes_id, mask))
            elif dataset_name == 'test_gallery' and self.test_with_mask:
                mask = os.path.join(self.test_gallery_mask_dir, img_path.split('/')[-2], img_path.split('/')[-1][:-4] + '.npy')
                dataset.append((img_path, pid, camid, clothes_id, mask))
            else:
