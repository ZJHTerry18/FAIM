...
class CrossEntropyWithClothes(nn.Module):
    ...
    def forward(self, inputs, id_targets, clothes_targets):
        """
        Args:
            inputs: prediction matrix (before softmax) with shape (batch_size, num_classes)
            targets: ground truth labels with shape (batch_size)
            positive_mask: positive mask matrix with shape (batch_size, num_classes). The clothes classes with 
                ...
                as negative clothes classes and their mask values in positive_mask are 0.
        """
        identity_mask = torch.zeros(inputs.size()).scatter_(1, clothes_targets.unsqueeze(1).data.cpu(), 1).cuda()
        positive_mask = torch.zeros(inputs.size())
        for i in range(inputs.size(0)):
            pids = id_targets.data.cpu().numpy()
            clothes_ids = clothes_targets.data.cpu().numpy()
            pos_id = pids[i]
            pos_clothes_id = clothes_ids[pids == pos_id]
            positive_mask[i][pos_clothes_id] = 1
