...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, num_cls_token=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class PromptTransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu =1.0, num_cls_token=1,
                 prompt_num=5, prompt_drop_rate=0.0, deep_prompt=True):
        super().__init__()
        ...
        val = math.sqrt(6. / float(3 * self.patch_embed.patch_size + embed_dim))
        self.prompt_embeddings = nn.Parameter(torch.zeros(1, prompt_num, embed_dim))
        nn.init.uniform_(self.prompt_embeddings, -val, val)
        if deep_prompt:
            self.deep_prompt_embeddings = nn.Parameter(torch.zeros(depth - 1, prompt_num, embed_dim))
    ...
    def incorporate_prompt(self, x):
        B = x.shape[0]
        # prepend prompt tokens before the image tokens (after cls token)
        x = torch.cat((
            x[:, :1, :],
            self.prompt_dropout(self.prompt_embeddings.expand(B, -1, -1), x[:, 1:, :]),
            x[:, 1:, :]), dim=1)
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                v = resize_pos_embed(v, self.pos_embed, self.patch_embed.num_y, self.patch_embed.num_x, self.num_cls_token)
            try:
...
def resize_pos_embed(posemb, posemb_new, hight, width, num_cls_token):
    ...
    # posemb_token = posemb_token.repeat(1, num_cls_token, 1)
    posemb = torch.cat([posemb_token, posemb_grid], dim=1)
    return posemb


def vit_base_patch16_224_PromptTransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, num_cls_token=1, **kwargs):
    model = PromptTransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, num_cls_token=num_cls_token, **kwargs)

    return model

def vit_large_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, num_cls_token=1, **kwargs):
    model = PromptTransReID(
