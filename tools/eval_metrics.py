...
def evaluate(distmat, q_pids, g_pids, q_camids, g_camids):
    ...
    for i in range(num_q):
        ...
        ap_tmp, CMC_tmp, valid_index = compute_ap_cmc(index[i], good_index, junk_index)
        ap_list[i] = ap_tmp
        cmc_list[i] = CMC_tmp
        index_list[i] = valid_index
        if CMC_tmp[0]==1:
            num_r1 += 1
        CMC = CMC + CMC_tmp
        AP += ap_tmp
    ...
    return CMC, mAP, cmc_list, ap_list, index_list
...
def evaluate_only_cloth_changing(distmat, q_pids, g_pids, q_camids, g_camids, q_clothids, g_clothids):
    num_q, num_g = distmat.shape
    index = np.argsort(distmat, axis=1) # from small to large
    ...
    for i in range(num_q):
        ...
        if good_index.size == 0:
            num_no_gt += 1
            continue
        # remove gallery samples that have the same pid, camid or clothid with query
        junk_index1 = np.intersect1d(query_index, camera_index)
        junk_index2 = np.intersect1d(query_index, cloth_index)
        junk_index = np.union1d(junk_index1, junk_index2)
