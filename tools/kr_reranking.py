...
def batch_torch_topk(qf, gf, all_pids=None, all_clothids=None, all_aps=None, k1=30, N=6000):
    m = qf.shape[0]
    n = gf.shape[0]
...
def kr_re_ranking(dataset, probFea, galFea, probFea_id, galFea_id, probFea_cloth, galFea_cloth, 
                  q_pids, g_pids, q_clothids, g_clothids, k1, k2, lambda_j, lambda_ci, lambda_ic, lambda_cic, id_ap=None):
    ...
    if lambda_ci > 0.0 and lambda_j > 0.0:
        invIndex_ci = []
        ...
        for i in range(qnum):
            ...
            #         if os.path.basename(dataset_all[i][0]) == '016_2_c10_004333.png':
            #             print('------------------------')
    ...
    if lambda_ic > 0.0 and lambda_j > 0.0:
        invIndex_ic = []
    ...
    if lambda_cic > 0.0 and lambda_j > 0.0:
    ...
    lambda_weight_ori = 1.0 - (lambda_ci + lambda_ic + lambda_cic) / 3
    final_dist = (original_dist + jaccard_dist_ori) * lambda_weight_ori + jaccard_dist
    # print(jaccard_dist)
    del original_dist
...
def kr_re_ranking_ori(probFea, galFea, k1, k2):
    ...
    all_num = query_num + galFea.size(0)
    ori_feat = torch.cat([probFea, galFea]).cuda()
    ori_rank = batch_torch_topk(ori_feat, ori_feat, k1=k1 + 1, N=6000)
    # del feat
    del probFea
    del galFea
    torch.cuda.empty_cache()  # empty GPU memory
